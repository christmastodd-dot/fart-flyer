...
//   2. resume() must also be called inside a user-gesture handler.
//   3. Both #1 and #2 must be synchronous — no async gaps.
//   4. After that, sound nodes can be created from anywhere (including callbacks).
//
// We therefore expose SFX.unlock() which game.js calls via a document-level
// capture-phase touchstart listener — the earliest possible moment.
...
const SFX = {
  ...
  // ── Variation 1: mid-pitch sawtooth + brown noise ──────────────────────────
  _fart1() {
    const ac  = this._ctx;
  ...
  // ── Variation 2: high-pitched squeaky fart ─────────────────────────────────
  _fart2() {
    const ac  = this._ctx;
    const t   = ac.currentTime;
